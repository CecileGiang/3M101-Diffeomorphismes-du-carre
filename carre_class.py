...
class fonc_diff_infini:
    ...
    def __init__(self, expr, expr_reci=None, t0=-1, t1=1, vars_sym=(sp.symbols("x y"))):
        self.expr = expr  # l'expression symbolique de ce diffeomorphisme
        self.expr_reci = expr_reci  # l'expression symbolique du diffeomorphisme reciproque
    ...
    def trace(self, temps=1, t0=-1, t1=1, taille=50, multi=10, precision=0.005):
        tab_angles = self.tab_angles_R(t0, t1, taille, multi) * temps
        axe = np.linspace(t0, t1, taille)
        res = []

        def find_sim_points(x__, y__):
            pas = (t1 - t0) / (taille - 1)
            kx0 = int((x__ - t0) // pas)
            if kx0 >= taille:
                kx0 = taille - 1
            kx1 = kx0 + 1 if kx0 < taille - 1 else kx0
            t = (x__ - t0 - kx0 * pas) / pas
            ky0 = int((y__ - t0) // pas)
            if ky0 >= taille:
                ky0 = taille - 1
            ky1 = ky0 + 1 if ky0 < taille - 1 else ky0
            s = (y__ - t0 - ky0 * pas) / pas
        ...
        def trace_v():
            ...
            for x_ in axe:
                ...
                tab_v.append([tab_trace_vx, tab_trace_vy])
