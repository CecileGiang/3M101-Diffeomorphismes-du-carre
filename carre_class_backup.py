...
class DiffeoInfini:
    ...
    def __init__(self, expr, expr_reci=None, t0=-1, t1=1, vars_sym=(sp.symbols("x y"))):
        ...
        _t0, _t1: (#)etant respectivement -1 et 1 par default, consistent a l'interval I tel que I = [_t0, _t1]
        _x, _y: etant respectivement x et y par default, representent les deux variables du diffeomophisme
        num: la fonction de python correspondant a expr
        num_reci: (*)la fonction de python correspondant a expr_reci
        """
        self.expr = expr
        self.expr_reci = expr_reci
        self._t0, self._t1 = t0, t1
        self._x, self._y = vars_sym
        self.num = sp.lambdify((self._x, self._y), self.expr, "numpy")
        ...
        df_sym: l'expression symbolique du differentiel de ce diffeomorphisme
        df_num: la fonction de python correspondant a df_sym
        df_reci_sym: (*)l'expression symbolique du differentiel du diffeomorphisme reciproque
        df_reci_num: (*)la fonction de python correspondant a df_reci_sym
        """
        self.df_sym = self.df_dim2_sym(self.expr, self._x, self._y)
        self.df_num = sp.lambdify((self._x, self._y), self.df_sym, "numpy")
        self.df_reci_sym = self.df_dim2_sym(self.expr_reci, self._x, self._y) if self.expr_reci is not None else None
        self.df_reci_num = sp.lambdify((self._x, self._y), self.df_reci_sym,
                                       "numpy") if self.df_reci_sym is not None else None
        """
        Variables sur les resultats sous forme de tableau :
        _plan: (#)meshes grid d'un plan, sous forme de meshgrid de 3 dimentions: (x ou y, ligne, colonne)
        _tab_f: (#)f(plan), sous forme de meshgrid de 3 dimentions
        _tab_df: (#)df(plan), sous forme de meshgrid de 4 dimenttions: (∂f1 ou ∂f2, ∂x ou ∂y, ligne, colonne)
        """
        self._plan = None
        self._tab_f = None
        self._tab_df = None
        # Les points dans l'ensemble depart dont images par ce diffeomorphisme sont au meshgrid de l'ensemble arrive
        # tableau en 3 dimentions (ligne,colonne,(x,y))
    ...
    def plan(self, t0=None, t1=None, taille=50):
        """
        ...
        if self.change_domain(t0, t1) or self._plan is None or taille != len(self._plan):
            axe = np.linspace(self._t0, self._t1, taille)
    ...
    def tab_f(self, t0=None, t1=None, taille=50):
    ...
    def tab_df(self, t0=None, t1=None, taille=50):
        """
